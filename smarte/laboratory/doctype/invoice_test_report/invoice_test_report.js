...
frappe.ui.form.on('Invoice Test Report', {
	setup: function(frm) {
		frm.get_field('lab_test_presult').grid.editable_fields = [
			{fieldname: 'print_this', columns: 1},
			{fieldname: 'lab_procedure', columns: 6},
			{fieldname: 'workflow', columns: 3},
		];
	},
	refresh :  function(frm){
		refresh_field('lab_test_presult');
		var doc = frm.doc;
		...
		for(result in doc.lab_test_presult){
			if((doc.lab_test_presult[result].workflow == "Submitted") && !approvable){
				if(doc.lab_test_presult[result].sms_sent==0){
					btn_send_sms_show = true;
				}
			}
			else if((doc.lab_test_presult[result].workflow == "Approved") && approvable){
				if(doc.lab_test_presult[result].sms_sent==0){
					btn_send_sms_show = true;
				}
			}
		...
		frm.add_custom_button(__("View Lab Procedures"), function() {
...
var mark_as_completed = function(frm){
	...
	for(result in doc.lab_test_presult){
		if(!(doc.lab_test_presult[result].workflow == "Submitted") && !approvable && !(doc.lab_test_presult[result].workflow == "Cancelled")){
			msgprint("All Procedures should be 'Submitted' or 'Cancelled' to complete")
			throw("Error");
		}
		else if(!(doc.lab_test_presult[result].workflow == "Approved") && approvable && !(doc.lab_test_presult[result].workflow == "Cancelled")){
			msgprint("All Procedures should be 'Approved' or 'Cancelled' to complete")
			throw("Error");
		}
	}
	frappe.call({
		method:"smarte.laboratory.doctype.invoice_test_report.invoice_test_report.mark_as_completed",
				...
				for(result in doc.lab_test_presult){
					if(doc.lab_test_presult[result].sms_sent==0){
						frappe.call({
							...
							name: doc.lab_test_presult[result].lab_procedure
							},
							callback: function(r){
								cur_frm.reload_doc();
							}
