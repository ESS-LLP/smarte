	...
	refresh: function(frm) {
		...
		if(frm.doc.status== 'Admitted'){
			...
			/*frm.add_custom_button(__("View All Consultations"), function() {
				frappe.route_options = {"inpatient_id": frm.doc.name }
...
var cancel_schedule = function(frm){
	...
	var d = new frappe.ui.Dialog({
		...
		primary_action_label : __("Cancel Inpatient"),
		primary_action : function(){
			frappe.call({
				"method": "smarte.ip.doctype.inpatients.inpatients.cancel_scheduled_inpatient",
				"args": {inpatient:doc.name},
				callback: function(r){
					cur_frm.reload_doc();
				}
			});
	...
	d.fields_dict.confirmation_messages.html("Are You Sure to Proceed ? Click 'Cancel Inpatient'".bold())
	d.show();
}
...
var admit_patient = function(frm){
	...
	var admit_and_allocate = function(date_in,time_in,bed_number,facility_type,facility_name,expected_discharge){
		frappe.call({
			"method": "smarte.ip.doctype.inpatients.inpatients.admit_and_allocate_patient",
			"args": {patient: doc.patient, inpatient: doc.name,
...
var allocate_patient = function(frm){
	...
	var allocate = function(date_in,time_in,bed_number,facility_type,facility_name,expected_discharge){
		frappe.call({
			"method": "smarte.ip.doctype.inpatients.inpatients.allocate_facility",
			"args": {patient: doc.patient, inpatient: doc.name,
				date_in:date_in, time_in:time_in, bed: bed_number,
				 facility_type: facility_type, facility: facility_name,
...
var queue_discharge_patient = function(frm){
	...
	var d = new frappe.ui.Dialog({
		...
		primary_action : function(){
			frappe.call({
				"method": "smarte.ip.doctype.inpatients.inpatients.queue_discharge_patient",
				"args": {patient: doc.patient, inpatient: doc.name},
				callback: function(r){
					cur_frm.reload_doc();
				}
...
var discharge_patient = function(frm){
	...
	var d = new frappe.ui.Dialog({
		...
		primary_action : function(){
			frappe.call({
				"method": "smarte.ip.doctype.inpatients.inpatients.discharge_patient",
				"args": {patient: doc.patient, inpatient: doc.name},
				callback: function(r){
					cur_frm.reload_doc();
				}
...
var btn_create_consultation = function(frm){
	var doc = frm.doc;
	frappe.call({
		method:"smarte.ip.doctype.inpatients.inpatients.create_consultation",
		args: {patient: doc.patient,inpatient:doc.name},
...
var btn_create_inv_for_facility_used = function(frm){
	var doc = frm.doc;
	frappe.call({
		method:"smarte.ip.doctype.inpatients.inpatients.create_inv_for_facility_used",
		args: {patient: doc.patient,inpatient:doc.name},
		callback: function(data){
			if(!data.exc){
				var doclist = frappe.model.sync(data.message);
...
var btn_discharge_summary = function(frm){
	var doc = frm.doc;
	frappe.call({
		method:"smarte.ip.doctype.inpatients.inpatients.create_discharge_summary",
		args: {inpatient:doc.name},
		callback: function(data){
			if(!data.exc){
...
frappe.ui.form.on("InPatients", "op_consultation_id",
    function(frm) {
...
frappe.ui.form.on("InPatients", "patient",
    function(frm) {
	if(frm.doc.patient){
		frappe.call({
		    "method": "frappe.client.get",
...
frappe.ui.form.on("InPatients", "physician",
    function(frm) {
